...
import { DiscountType, GetProductSearchInStockSelectListQuery, ProductsClient, ProductSelectListModel, SaleDetailModel, SaleModel, SalesClient, TaxMethod, UpsertSaleModel } from 'src/app/modules/generated-clients/api-service';
...
@Component({
  ...
  providers: [SalesClient, DatePipe, ProductsClient]
})
export class SaleDetailComponent implements OnInit {
  ...
  */
  ...
  private increaseQuantityIfProductAlreadyExist(product: ProductSelectListModel): boolean {
    let existedProduct = this.item.saleDetails.find(x => x.productId === product.id);
    if (existedProduct) {
